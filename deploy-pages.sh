...
echo "🔨 Building production bundle + pre-rendered pages..."
# build:full also writes sitemap.xml and the not-found 404.html into dist/
npm run build:full
...
TMP=$(mktemp -d)
cp -r dist/. "$TMP"/
# dist/404.html is generated by tools/prerender.js (the not-found page, noindex) —
# never overwrite it with the legacy src/404.html stub.
